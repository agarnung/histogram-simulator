...
#include "ui_histogramsimulatorui.h"
...
HistogramSimulatorUI::HistogramSimulatorUI(QWidget* parent)
    : QWidget(parent)
	, mUi(new Ui::HistogramSimulatorUI)
{
    mUi->setupUi(this);
...
HistogramSimulatorUI::~HistogramSimulatorUI()
{
	if (mUi)
	{
		delete mUi;
...
void HistogramSimulatorUI::initializeSimulator()
{
	...
	if (styleFile.open(QFile::ReadOnly | QFile::Text))
	{
        ...
        qDebug() << QObject::tr("%1 - %2 - No se pudo abrir la hoja de estilos").arg(this->metaObject()->className()).arg(__func__);

	setupView();
	this->update();
}

void HistogramSimulatorUI::readJson(const QString& jsonPath)
{
    // QString json = ...;
    // mLUT = parseJSONQStringToMat(json);
}

void HistogramSimulatorUI::loadImageInView()
{
...
void HistogramSimulatorUI::loadHistogramInView()
{
...
void HistogramSimulatorUI::updateView(QPointF* skipPoint)
{
	loadHistogramInView();
...
void HistogramSimulatorUI::interpolateCurve()
{
...
void HistogramSimulatorUI::on_deleteLastPointButton_released()
{
...
void HistogramSimulatorUI::on_saveTfButton_released()
{
//	createLUT();
	saveJSON(); // Parsear LUT a JSON y guardar
}

void HistogramSimulatorUI::on_addPointButton_released()
{
...
QImage HistogramSimulatorUI::calculateHistogram(const cv::Mat& image)
{
	int histSize = 256;
	float range[] = { 0, 256 } ;
...
QImage HistogramSimulatorUI::createHistogramImage(cv::Mat histImageMat, int histSize)
{
	int hist_w = 1200;
	int hist_h = 800;
...
void HistogramSimulatorUI::createLUT()
{
	interpolateCurve();
...
void HistogramSimulatorUI::saveJSON()
{
...
void HistogramSimulatorUI::setupView()
{
...
void HistogramSimulatorUI::fillKnotVector()
{
...
void HistogramSimulatorUI::on_saveImagesButton_released()
{
	...
	if (!directory.isEmpty())
	{
		...
		{
			QString originalPath = QDir(dir).filePath("HistogramSimulatorUI_mOriginal.png");
			QString modifiedPath = QDir(dir).filePath("HistogramSimulatorUI_mModified.png");
...
ControlPoint::ControlPoint(qreal x, qreal y, qreal width, qreal height, HistogramSimulatorUI* histogramSimulatorUI, QGraphicsItem* parent, QString name)
	: QGraphicsEllipseItem(x, y, width, height, parent), mHistogramSimulatorUI(histogramSimulatorUI)
{
...
void ControlPoint::mouseMoveEvent(QGraphicsSceneMouseEvent* event)
{
	...
	QPointF *point = mHistogramSimulatorUI->itemToPoint[this];

	*point = pos.toPoint();
	const QList<QGraphicsItem*> &items = mHistogramSimulatorUI->mScene->items();
	for (QList<QGraphicsItem*>::const_iterator itemIt = items.begin(); itemIt != items.end(); ++itemIt)
	{
		if (*itemIt != this)
		{
			mHistogramSimulatorUI->mScene->removeItem(*itemIt);
			//mainWindow->itemToPoint.remove(static_cast<MovingEllipseItem*>(*itemIt));
			delete *itemIt;
		}
	}
	mHistogramSimulatorUI->updateView(point);
}

void HistogramSimulatorUI::on_leftSlider_valueChanged(int position)
{
...
void HistogramSimulatorUI::on_rightSlider_valueChanged(int position)
{
	if (!mScene || mControlPoints.empty())
		return;
...
void HistogramSimulatorUI::on_cargarButton_released()
{
...
void HistogramSimulatorUI::on_transformButton_released()
{
	if (mLUT.empty())
		return;
...
void HistogramSimulatorUI::on_YButton_released()
{
	bool ok;
...
void HistogramSimulatorUI::on_leftVerticalSlider_valueChanged(int position)
{
	if (!mScene || mControlPoints.empty())
		return;
...
void HistogramSimulatorUI::on_rightVertSlider_valueChanged(int position)
{
	if (!mScene || mControlPoints.empty())
		return;
...
void HistogramSimulatorUI::on_comboBox_activated(const QString& arg1)
{
...
void HistogramSimulatorUI::on_helpButton_released()
{
	if (!mHelpDialog)
	{
		QString helpMessage;
...
void HistogramSimulatorUI::on_createLUTControl_released()
{
	if (!mScene || mControlPoints.empty())
		return;
...
void HistogramSimulatorUI::on_saveCurveButton_released()
{
	QJsonArray jsonArray;
...
void HistogramSimulatorUI::on_loadCurveButton_released()
{
	if (!mScene)
		return;
...
cv::Mat HistogramSimulatorUI::parseJSONQStringToMat(QString& json)
{
